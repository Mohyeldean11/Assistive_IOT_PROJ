...
def load_model(model_path: Optional[Path | str] = None):
...
def save_model(model, model_path: Optional[Path | str] = None):
    path = Path(model_path) if model_path is not None else DEFAULT_MODEL_PATH
...
def save_training_sample(pose_sequence: list[dict], label: str, dataset_path: Optional[Path] = None):
    ...
    row: dict[str, object] = dict(zip(FEATURE_COLUMNS, vectorize_features(features)))
    row['label'] = str(label)
